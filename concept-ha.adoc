...
NOTE: BlueXP創建 https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/placement-groups.html["AWS 文件：AWS 分佈置放群組"^]並啟動該放置組中的兩個 HA 節點。放置組透過將實例分佈在不同的底層硬體上來降低同時發生故障的風險。此功能從計算角度而不是從磁碟故障角度提高了冗餘度。
...
有關最新和完整的詳細信息，請參閱 AWS 文檔link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["AWS 文件：本地區域中的 EC2 執行個體類型"^]。
