...
Cloud Volumes ONTAP 僅支援 AWS 本機區域中的 HA 部署。以下為範例組態：
...
Cloud Volumes ONTAP 僅支援這些組態。在 AWS 本機區域組態中為 Cloud Volumes ONTAP 選取任何其他磁碟類型或 EC2 執行個體、可能會導致部署失敗。請參閱 AWS 文件以取得最新完整的詳細資料link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["本機區域中的 EC2 執行個體類型"^]。
