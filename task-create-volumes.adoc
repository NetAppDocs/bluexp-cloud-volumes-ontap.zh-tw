...
* 指定新磁碟區的詳細資料、讓BlueXP為您處理基礎資料集合體。  a volume,深入瞭解。
...
* 建立iSCSI磁碟區時、BlueXP會自動為您建立LUN。我們只要在每個磁碟區建立一個 LUN 、就能輕鬆完成工作、因此不需要管理。建立磁碟區之後、  a LUN to a host,使用 IQN 從主機連線至 LUN。
