...
BlueXP會自動將所需的私有IP位址分配給Cloud Volumes ONTAP GCP中的某些人。您必須確定網路有足夠的私有位址可供使用。

Cloud Manager分配Cloud Volumes ONTAP 給功能不全的生命量取決於您是部署單一節點系統或HA配對。LIF 是與實體連接埠相關聯的 IP 位址。諸如 VMware 的管理工具需要 SVM 管理 LIF SnapCenter 。
...
TIP: 正在尋找Connector的相關資訊？ https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-ports-gcp.html["檢視Connector的防火牆規則"^]
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-gcp.html["檢視連接器的網路需求"^]
