...
連接器應部署在Cloud Volumes ONTAP 其所管理的或所管理的各個系統所在的Azure區域 https://["Azure區域配對"^] 適用於整個系統。Cloud Volumes ONTAP
...
https://["瞭解如何在Connector上啟用直接API流量"^]
...
如果您使用 https://["Azure 私有 DNS"^]然後您需要修改每個 Connector 上的組態檔。否則、BlueXP無法在Cloud Volumes ONTAP 支援的儲存帳戶之間啟用Azure Private Link連線。

請注意、 DNS 名稱必須符合 Azure DNS 命名需求 https://["如 Azure 文件所示"^]。
