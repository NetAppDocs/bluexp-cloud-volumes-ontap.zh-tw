...
支援大部分Google Cloud地區的支援。Cloud Volumes ONTAP https://["檢視支援區域的完整清單"^]。
...
https://["支援的GCP組態Cloud Volumes ONTAP"^]
...
https://["適用於GCP的儲存限制Cloud Volumes ONTAP"^]
...
機器類型:: 請查看中支援的機器類型 http://["發行說明 Cloud Volumes ONTAP"^] 然後檢視 Google 提供的每種受支援機器類型的詳細資料。將工作負載需求與機器類型的 vCPU 和記憶體數量配對。請注意、每個 CPU 核心都能提升網路效能。
+
--
如需詳細資料、請參閱下列內容：

* https://["Google Cloud 文件： N1 標準機器類型"^]
* https://["Google Cloud 文件：效能"^]
...
+
如需詳細資料、請參閱 https://["Google Cloud 文件：分區持續磁碟（標準和 SSD ）"^]。
...
+
** https://["Google Cloud 文件：分區持續磁碟（標準和 SSD ）"^]
** https://["Google Cloud 文件：最佳化持續磁碟和本機 SSD 效能"^]
