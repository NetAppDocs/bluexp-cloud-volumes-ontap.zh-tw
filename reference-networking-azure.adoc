...
NOTE: NIC 3僅適用於具有分頁大型資源配置系統的HA部署。
...
根據預設、BlueXP會啟用Azure Private Link、以便Cloud Volumes ONTAP 在支援的範圍內連線到Azure儲存帳戶。

在大多數情況下、您完全不需要做任何事、而是由NetApp為您管理Azure Private Link。但如果您使用Azure私有DNS、則必須編輯組態檔。您也應該瞭解Azure中的Connector位置需求。
...
您不需要建立安全性群組、因為BlueXP會為您建立安全性群組。如果您需要使用自己的安全性群組規則、請參閱下列安全性群組規則。
