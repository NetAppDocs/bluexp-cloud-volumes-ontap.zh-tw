...
summary: 使用OpenSSL驗證Google Cloud下載的disk.原始 檔案是否符合摘要檔案內容。 
---
= 使用 Cloud Volumes ONTAP 驗證 Google Cloud 映像 disk.raw 檔案
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
您可以根據可透過取得的摘要檔案內容、驗證Google Cloud下載的disk.RAW檔案 https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab["NSS"^] 使用OpenSSL。


NOTE: 用於驗證映像的 OpenSSL 命令與 Linux、macOS 和 Windows 機器相容。
