...
+
從 Cloud Volumes ONTAP 9.15.1 開始、您可以在本機備援儲存設備（ LRS ）單一可用性區域中部署 VM 執行個體。這也需要 `Microsoft.Compute/VMOrchestratorZonalMultiFD`為您的訂閱啟用此功能。此功能可協助您在相同的可用性區域中部署高可用度（ HA ）配對。請參閱link:task-saz-feature.html["為單一可用性區域啟用 VMOrchestratorZonalMultiFD"]和link:task-azure-high-availability-mode.html["在Azure中啟用高可用度模式"]。
...
+
*** 針對單一可用性區域、請選取 Azure 區域、可用性區域、 vnet 和子網路。從 Cloud Volumes ONTAP 9.15.1 開始、您可以在單一可用區域中安裝 VM 執行個體。您需要選取支援此模式的區域。如果您選取的區域不支援分區部署、則會遵循先前的 LRS 非分區部署模式。
+
若要深入瞭解此部署模式和支援的 Azure 區域、請參閱 https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Microsoft Azure 文件： Virtual Machine Scale Set 文件"^]和 https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Microsoft Azure 網站：依地區提供的產品"^]。
