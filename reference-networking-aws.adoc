...
| AWS 服務的特定政府端點取決於您使用的 AWS 區域。端點後綴為 `amazonaws.com`和 `c2s.ic.gov`。參考	https://docs.aws.amazon.com/AWSJavaSDK/latest/javadoc/com/amazonaws/services/s3/model/Region.html["AWS SDK"^]和 https://docs.aws.amazon.com/general/latest/gr/rande.html["AWS 文件"^]了解更多。  a| 
* CloudForation
* 彈性運算雲端（ EC2 ）
* 身分識別與存取管理（ IAM ）
* 金鑰管理服務（ KMS ）
