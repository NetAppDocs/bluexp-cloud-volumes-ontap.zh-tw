...
WORM （一次寫入多次讀取）與 ARP （自主勒索軟體保護）的內建資料保護與安全功能、將免費隨附於所有 Cloud Volumes ONTAP 授權中。這將適用於節點型和容量型授權的全新和現有 BYOL 和 PAYGO/ 市場部署。新的定價模式適用於所有雲端供應商和部署類型、包括單一節點和高可用度（ HA ）組態。

將提供下列優點：

* 使用 WORM 和 ARP 的現有帳戶將不再收取這些功能的費用。您將依此變更之前適用的容量使用量計費。WORM 和 ARP 將在未來計費中排除。
* 如果您的現有帳戶中沒有啟用這些功能、現在您可以選擇 WORM 和 ARP 、而無需額外付費。
* 對於新帳戶、所有 Cloud Volumes ONTAP 產品都將免除 WORM 和 ARP 費用。
