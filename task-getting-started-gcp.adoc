...
如果您沒有 https://["連接器"^] 然而、帳戶管理員需要建立一個帳戶。 https://["瞭解如何在Google Cloud中建立Connector"^]
...
. 如果您打算啟用資料分層、 https://["設定Cloud Volumes ONTAP 私有Google Access的子網路"^]。
...
[role="quick-margin-para"]
https://["在專案中啟用下列 Google Cloud API"^]。這些 API 是部署連接器和 Cloud Volumes ONTAP 功能不全的必備條件。
...
* https://["BlueXP使用Google Cloud權限的功能"^]
