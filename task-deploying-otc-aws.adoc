...
| 編輯認證資料 | 選擇 AWS 認證資料和市場訂閱、以搭配此 Cloud Volumes ONTAP 款功能系統使用。按一下「新增訂閱」將選定的憑證與新的 AWS 市場訂閱關聯。訂閱可以是一年一度的合約、或Cloud Volumes ONTAP 是以每小時的費率支付。如果您直接向 NetApp 購買授權（請自帶授權（ BYOL ）），則不需要 AWS 訂閱。https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-aws-accounts.html["瞭解如何將額外的AWS認證資料新增至BlueXP"^]。 
|===
+
以下影片顯示如何將按使用量付費的市場訂閱與您的 AWS 憑證關聯：
