...
* 您不需要在系統層級啟用資料分層。
+
Cloud Manager 會在系統建立時、為 Cold 資料建立物件存放區、  data tiering after implementing the requirements,只要沒有連線或權限問題。之後、您只需要在磁碟區上啟用資料分層功能（在某些情況下、  that tiering is enabled on aggregates,在 Aggregate 上）。
