...
summary: 與標準AWS區域類似、您可以在AWS商業雲端服務（C2S）環境中使用BlueXP（前身為Cloud Manager）來部署Cloud Volumes ONTAP 支援功能、為您的雲端儲存設備提供企業級功能。AWS C2S是美國專屬的封閉區域智慧社群；本頁的說明僅適用於AWS C2S區域使用者。 
---
...
[role="lead"]
與標準AWS區域類似、您可以在中使用BlueXP（前身為Cloud Manager） https://aws.amazon.com/federal/us-intelligence-community/["AWS商業雲端服務（C2S）"] 部署Cloud Volumes ONTAP 的環境、為您的雲端儲存設備提供企業級功能。AWS C2S是美國專屬的封閉區域智慧社群；本頁的說明僅適用於AWS C2S區域使用者。



== C2S支援的功能

在C2S環境中、可從BlueXP取得下列功能：
...
. 安裝Connector並設定BlueXP。
+
在開始使用BlueXP部署Cloud Volumes ONTAP 功能時、您必須先建立_Connector_。Connector讓BlueXP能夠管理公有雲環境中的資源和程序（包括Cloud Volumes ONTAP 整個過程）。

+
您將從安裝在Connector執行個體上的軟體登入BlueXP。

. 從Cloud Volumes ONTAP BlueXP啟動
...
需要訂閱Marketplace才能從Cloud Volumes ONTAP BlueXP部署功能。
...
+
您必須使用BlueXP來啟動Cloud Volumes ONTAP 實體執行個體。您不得Cloud Volumes ONTAP 從EC2主控台啟動支援的執行個體。
...
設定IAM原則和角色、為BlueXP和Cloud Volumes ONTAP IXP提供在AWS商業雲端服務環境中執行動作所需的權限。
...
+
從BlueXP建立一個可運作的環境時、您可以選擇IAM角色做Cloud Volumes ONTAP 為功能性的部分、以及HA中介器Cloud Volumes ONTAP 。
...
. 將BlueXP執行個體的IAM角色新增至CMK的主要使用者清單。
+
...
您Cloud Volumes ONTAP 必須先從AWS Marketplace啟動Connector執行個體、然後登入並設定BlueXP、才能在AWS中啟動更新系統。
...
+
.. 開啟網頁瀏覽器並輸入下列 URL ： http://ipaddress[]
.. 指定用於連線至AWS服務的Proxy伺服器。
.. 上傳您在步驟1中取得的憑證。
.. 完成設定精靈中的步驟以設定BlueXP。
+
*** *系統詳細資料*：輸入此BlueXP執行個體的名稱、並提供您的公司名稱。
*** *建立使用者*：建立管理使用者、以用於管理BlueXP。
...
您可以Cloud Volumes ONTAP 在BlueXP中建立新的工作環境、在AWS商業雲端服務環境中啟動執行個體。
...
BlueXP會建立安全性群組、其中包括BlueXP和Cloud Volumes ONTAP 不必在雲端上順利運作的傳入和傳出規則。您可能想要參照連接埠進行測試、或是想要使用自己的安全性群組。
...
針對此功能、預先定義 Cloud Volumes ONTAP 的內部安全群組包含下列規則：BlueXP一律會建立此安全性群組。您沒有使用自己的選項。
