...
從功能部件支援的版本起、您可以選擇在Volume層級啟用或停用WORM Cloud Volumes ONTAP 。您選擇如何啟用WORM取決於您的需求。
...
+
如果您想要建立WORM儲存專用系統、這個選項是很好的選擇。

* 如果您在建立工作環境時停用WORM儲存設備、則您從BlueXP建立的每個Volume都會停用WORM。但您可以使用System Manager或CLI建立已啟用WORM的磁碟區。
+
如果您想在同一個系統上混合不同類型的磁碟區、這個選項是很好的選擇。
...
您可以在 Cloud Volumes ONTAP 建立新的工作環境時、在一個可靠的系統上啟動 WORM 儲存設備。您無法在個別磁碟區上啟動WORM儲存設備、WORM必須在系統層級啟動。只有在您建立工作環境時才能啟動。
