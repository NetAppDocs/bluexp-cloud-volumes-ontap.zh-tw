...
您應該在 Azure 入口網站上設定加密金鑰。如需設定金鑰和設定金鑰旋轉的相關資訊，請參閱下列 Microsoft Azure 文件主題：
...
NOTE: 設定金鑰之後，請確定您擁有 https://learn.microsoft.com/en-us/azure/virtual-machines/disk-encryption#customer-managed-keys["已在 Azure 入口網站上啟用金鑰旋轉"^]，以便 Cloud Volumes ONTAP 能夠使用新的金鑰。

在建立及啟用新版金鑰時， Cloud Volumes ONTAP 會自動偵測並使用最新版的加密功能，確保您的資料安全無虞，無需手動更新。
