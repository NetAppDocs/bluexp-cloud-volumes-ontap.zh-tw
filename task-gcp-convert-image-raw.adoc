...
---
= 將 Cloud Volumes ONTAP 的 Google Cloud 映像轉換為原始格式
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
無法直接從Google Cloud下載影像。若要根據簽署的摘要和憑證來驗證映像、您需要有機制來比較這兩個檔案並下載映像。若要這麼做，您必須將影像匯出 / 轉換成磁碟。原始格式，並將結果儲存在 Google Cloud 的儲存庫中。磁碟.RAW檔案會在處理過程中產生損及壓縮。
...
NOTE: 如需如何透過 Google Cloud 匯出影像的詳細資訊、請 https://cloud.google.com/compute/docs/images/export-image#console["匯出影像的Google Cloud文件"^]參閱。
