...
大多數Microsoft Azure地區均支援此功能。Cloud Volumes ONTAP https://["檢視支援區域的完整清單"^]。
...
https://["Azure支援Cloud Volumes ONTAP 的支援功能組態"^]
...
https://["Azure的Cloud Volumes ONTAP 儲存限制"^]
...
虛擬機器類型:: 請查看中支援的虛擬機器類型 http://["發行說明 Cloud Volumes ONTAP"^] 然後檢閱每種受支援 VM 類型的詳細資料。請注意、每種 VM 類型都支援特定數量的資料磁碟。
+
--
* https://["Azure 文件：通用虛擬機器大小"^]
* https://["Azure 文件：記憶體最佳化的虛擬機器大小"^]
...
如需這些磁碟使用案例的其他詳細資料、請參閱 https://["Microsoft Azure 文件： Azure 提供哪些磁碟類型？"^]。
...
* https://["Microsoft Azure ：託管磁碟定價"^]
* https://["Microsoft Azure ：網頁 Blobs 定價"^]
